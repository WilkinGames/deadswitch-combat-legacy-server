...
class GameInstance
{
    init(_data, _onEventFunc)
    {
        ...
        this.game = {
            ...
            gameModeData: {
                ...
                vehicleRespawnTime: _data.settings.vehicleRespawnTime ? _data.settings.vehicleRespawnTime : 60,
                weaponRespawnTime: _data.settings.weaponRespawnTime ? _data.settings.weaponRespawnTime : 60,
                bAllowRespawns: true,
                bHardcore: _data.settings.bHardcore == true,
                bFallDamage: _data.settings.bFallDamage == true,
        ...
        if (seats)
        {
            ...
            if (weapons)
            {
                ...
                if (ps)
                {
                    if (ps.vehicles)
                    {                        
                        var playerWeapon = ps.vehicles[_vehicle.data.vehicleId];
                        if (playerWeapon && playerWeapon.weapons)
                        {                            
                            var newData = this.getWeaponData(playerWeapon.weapons[_seatIndex]);
                            if (newData)
                            {
    ...
    handleAICharacter(_body)
    {
        ...
        {            
            ...
            if (ai.enemy.data.type == "character" && ai.enemyDist < data.melee.range)
            {
        ...
        if (ai.botSkill >= BotSkill.SKILL_HARD)
        {
            ...
            this.setDataValue(_body, "bCrouching", !data.bWantsToMove && !data.bClimbing && !data.bSprinting);
        }
        switch (this.game.gameModeId)
        {
    ...
    setVehicleWeaponAimRotation(_scale, _weapon, _val)
    {
        if (_weapon)
        {
            ...
            if (_weapon.minAngle != null)
            {
                ...
                {
                    val = Math.max(_weapon.minAngle - this.ToRad(180), val);
                }
            }
    ...
    handleControllableInput(_char, _controllable, _data)
    {
        ...
        switch (type)
        {
            ...
            case "tank":
                var tankSpeed = 250 / this.game.fpsMult;
                switch (keyId)
                {
                    case Control.JUMP:
                        if (_char.data.seatIndex == 0)
            ...
            case "helicopter":
                var heliSpeed = 4500 / this.game.fpsMult;
                switch (keyId)
                {
                    case Control.JUMP:
                        if (_char.data.seatIndex == 0)
                    ...
                    switch (dataB.type)
                    {
                        ...
                        case "platform":
                            ...
                            if (!dataA.controllableId && !dataA.bClimbing && !dataA.bLadderCooldown)
                            {
                                ...
                                if (this.game.gameModeData.bFallDamage)
                                {
                                    if (_bodyA.velocity)
                                    {
